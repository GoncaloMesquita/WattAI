...
from buffer_pretrain import ReplayBuffer
from Networks_pre_training import ActorNetwork, CriticNetwork, ValueNetwork

class Agent():
    def __init__(self, alpha=0.0003, beta=0.0003, input_dims=[8],
            env=None, gamma=0.99, n_actions=2, max_size=1000000, tau=0.001,
            layer1_size=256, layer2_size=256, batch_size=256, reward_scale=2, chkpt_dir='RL_pretraining/sac'):
        self.gamma = gamma
        self.tau = tau
        ...
        self.actor = ActorNetwork(alpha, input_dims, n_actions=n_actions,
                    name='actor', max_action=env.action_space.high, min_action= env.action_space.low, chkpt_dir=chkpt_dir)
    ...
    def remember(self, state, action, real_action, reward, new_state, done):
        self.memory.store_transition(state, action,real_action, reward, new_state, done)
    ...
    def learn(self,scaler):
        if self.memory.mem_cntr < self.batch_size:
            return

        state, action, real_actions, reward, new_state, done = \
        ...
        real_actions = T.tensor(real_actions, dtype=T.float).to(self.actor.device)
        real_actions.requires_grad = True
